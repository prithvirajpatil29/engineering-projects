...
void drawCar()
{
	...
	// 자동차 천장 채우기 // car ceiling filling
	glBegin(GL_POLYGON);
		glVertex3f(-0.1, 0.6, 0.7);//o
		glVertex3f(-0.1, 0.6, -0.7);//o'
		glVertex3f(-1.02, 0.6, -0.7);//p'
	...
	//자동차 뒷면 채우기 //fill the back of the car
	glBegin(GL_POLYGON);
		glVertex3f(-1.2, 0.22, 0.7);//q
		glVertex3f(-1.2, 0.22, -0.7);//q'
		glVertex3f(-1.2, -.28, -0.7);//r'
	...
	// 바퀴 이음새부분 채우기 //Filling the wheel seams
	glBegin(GL_POLYGON);
	...
	//왼쪽 뒷바퀴 부분 비운곳 채우기 //Filling the empty space on the left rear wheel
	glBegin(GL_POLYGON);
		glVertex3f(-1.12, -.48, -0.7);//a'
		glVertex3f(-0.86, -.48, -0.7);//b'
		glVertex3f(-.74, -0.2, -0.7);//c'
	...
	// 천장과 유리가 맞닿은곳 채우기 //Fill in the space where the ceiling meets the glass
	glBegin(GL_POLYGON);
		glVertex3f(-1.02, 0.6, -0.7);//p'
		glVertex3f(-0.98, 0.5, -0.7);//aa'
		glVertex3f(0.44, 0.5, -0.7);//jj'
	...
	// 후방에서 볼때 앞에 비어있는 곳 채우기 // Filling in the blank in the front when viewed from the rear
	glBegin(GL_POLYGON);
		glVertex3f(1.52, .14, 0.7);//k
		glVertex3f(1.52, .14, -0.7);//k'
		glVertex3f(1.52, -.44, -0.7);//j'
	...
	// 타이어그리기 //tire drawing
	glTranslatef(-.58, -.52, 0.7);// 첫번째 타이어 translate //first tire
	glColor3f(0.1, 0.1, 0.1);// 타이어 색 //tire color
	glutSolidTorus(0.12f, .14f, 10, 25);
	glTranslatef(1.68, 0.0, 0.0);// 두번째 타이어 translate //second tire
	glutSolidTorus(0.12f, .14f, 10, 25);

	glTranslatef(0.0, 0.0, -1.4);// 세번째 타이어 translate //third tire
	glutSolidTorus(0.12f, .14f, 10, 25);
	glTranslatef(-1.68, 0.0, 0.0);// 네번째 타이어 translate //fourth
	glutSolidTorus(0.12f, .14f, 10, 25);
	glTranslatef(.58, .52, 0.7);// 원본 타이어 translate //original
	glRotatef(90.0, 0.0, 1.0, 0.0);
	glTranslatef(0.0, 0.0, -1.40);

	glutSolidTorus(0.2f, .2f, 10, 25); // 차 뒤에 달린 타이어. 장식용. //tires on the back of the car. For decoration.
	...
	// 자동차 창문 코드 //car window code
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);// 유리는 반투명 //glass is translucent

	// 전방유리//windscreen //front windshield
	glBegin(GL_POLYGON);
	...
	// 오른쪽 뒷자석 유리 //right rear seat glass
	glBegin(GL_POLYGON);
	glVertex3f(-0.98, 0.5, 0.7);//aa
	glVertex3f(-0.64, 0.5, 0.7);//bb
	glVertex3f(-0.64, 0.22, 0.7);//cc
...
void drawLine() { // 주차선 그리는 함수 //Parking line drawing function
	glLineWidth(5);
...
GLuint DisplayList() { // 리스트를 사용하여 속도향상. 인터넷에서 찾음 //Speed ​​up using lists. //found on the internet
...
void initScene()
{
	glEnable(GL_DEPTH_TEST); // 깊이 버퍼 형성 //Depth Buffer Formation
...
void renderScene(void) {
	...
	glClearColor(0.7, 0.85, 1.0, 1.0); // 하늘색 //cerulean
	...
	glEnd();
	drawLine();
	// 주차장에 있는 차들을 그리기 //drawing cars in parking lot
	for (int i = -2; i < 2; i++) {
		glPushMatrix();
	...
	if (fxincr>0 && fzincr<0) { // 자동차가 회전을 할수 있도록 해줌. //Allows the car to turn
		theta1 = -theta1;
	}
	...
	glPushMatrix(); // 움직이는 차. 주차해야되는 차를 그린다. //moving car. Draw a car that needs to be parked
...
void orientMe(float ang) { // 카메라 왼쪽, 오른쪽 돌아가는 함수//function to rotate camera left and right
	lx = sin(ang);
	lz = -cos(ang);
	glLoadIdentity();
	gluLookAt(x, y, z,
...
void orientMe2(float ang) { // 카메라 위 아래로 돌아가는 함수 //function to rotate the camera
	ly = sin(ang);
	lz = -cos(ang);
	glLoadIdentity();
	gluLookAt(x, y, z,
...
void moveMeFlat(int i) { // 카메라 x, z 축을 따라 이동시키는 함수 //Function to move the camera along the x and z axes
	if (xxxx == 1)
		y = y + i*(lz)*0.1;
...
void processNormalKeys(unsigned char key, int x, int y) { // 화면 이동함수, 기능 함수 Screen movement function, function function
	glLoadIdentity();
...
void cameraKey(int key, int x, int y) { // 화면 이동함수 2 screen movement function 2w
...
void moveCar(int key, int x, int y) {
	switch (key) {
		case GLUT_KEY_LEFT: // 왼쪽으로 전진하면서 돔  Dome while advancing to the left
			temp = fxincr;
		...
		case GLUT_KEY_RIGHT: // 오른쪽으로 전진하면서 돔  //Dome while advancing to the right
			temp = fxincr;
			fxincr = fxincr*cos(-theta) + fzincr*sin(-theta);
			fzincr = -temp*sin(-theta) + fzincr*cos(-theta);
			fx += fxincr;
		...
		case GLUT_KEY_UP: // 전진 up
			fx += fxincr;
			fz += fzincr;
			break;

		case GLUT_KEY_DOWN: // 후진 down
			fx -= fxincr;
			fz -= fzincr;
			break;
		}
...
void ProcessMenu(int value) { // 디스플레이 재 호출 Display Recall
 ...
 //bitmap_output(-0.6,-1.5,0.50,"PLEASE PRESS p TO START");
	//glutPostRedisplay();
}

void ProcessMenu1(int value) { // 메뉴콜백 함수 menu callback function
	switch (value) {
		case 1:	 // 만약 "차 움직이기" 를 메뉴에서 누르면 실행. If you press "Move Car" in the menu, it will run.
			if (moveCarVar == 0) { // var의 값이 0이었다면, if value is 0
			glutSpecialFunc(moveCar); // moveCar 함수를 실행함으로서 차 움직임. Car movement by executing a function
			moveCarVar = 1;
			}
			   else { // 그게 아니면 방향키로 카메라 기능 수행 //Otherwise, perform camera functions with the arrow keys.
